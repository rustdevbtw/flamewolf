...
int main() {
  ...
  diplomat_result_box_ICU4XLocale_ICU4XError locale_result =
      ICU4XLocale_create_from_string("bn", 2);
  ...
  if (!success) {
    printf(
        "Failed to write result of FixedDecimalFormatter::format to string.\n");
    return 1;
  }
  printf("Output is %s\n", output);
  ...
  diplomat_result_box_ICU4XFixedDecimal_ICU4XError fd_result =
      ICU4XFixedDecimal_create_from_string("1000007.070", 11);
  if (!fd_result.is_ok) {
