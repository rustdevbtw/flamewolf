...
int main() {
  ...
  ICU4XIsoDateTime date =
      ICU4XIsoDateTime::create(2022, 07, 11, 13, 06, 42, 0).ok().value();

  ICU4XTimeFormatter tf =
      ICU4XTimeFormatter::create_with_length(dp, locale, ICU4XTimeLength::Short)
          .ok()
          .value();
  ...
  ICU4XGregorianDateFormatter df =
      ICU4XGregorianDateFormatter::create_with_length(dp, locale,
                                                      ICU4XDateLength::Full)
          .ok()
          .value();
  ...
  ICU4XGregorianDateTimeFormatter dtf =
      ICU4XGregorianDateTimeFormatter::create_with_lengths(
          dp, locale, ICU4XDateLength::Medium, ICU4XTimeLength::Short)
          .ok()
          .value();
  ...
  ICU4XDateTime any_date = ICU4XDateTime::create_from_iso_in_calendar(
                               2020, 10, 5, 13, 33, 15, 0, cal)
                               .ok()
                               .value();
  ICU4XDateTimeFormatter any_dtf =
  ...
  ICU4XCustomTimeZone time_zone =
      ICU4XCustomTimeZone::create_from_string("-06:00").ok().value();
  int32_t offset = time_zone.gmt_offset_seconds().ok().value();
  if (offset != -21600) {
    std::cout << "GMT offset doesn't parse" << std::endl;
    return 1;
  }
  ICU4XMetazoneCalculator mzcalc =
      ICU4XMetazoneCalculator::create(dp).ok().value();
  ICU4XIanaToBcp47Mapper mapper =
      ICU4XIanaToBcp47Mapper::create(dp).ok().value();
  ...
  if (time_zone_id_return != "uschi") {
    std::cout << "Time zone ID does not roundtrip: " << time_zone_id_return
              << std::endl;
    return 1;
  }
  ICU4XBcp47ToIanaMapper reverse_mapper =
      ICU4XBcp47ToIanaMapper::create(dp).ok().value();
  std::string recovered_iana_id = reverse_mapper.get("uschi").ok().value();
  if (recovered_iana_id != "America/Chicago") {
    std::cout << "Time zone ID does not canonicalize to IANA: "
              << recovered_iana_id << std::endl;
    return 1;
  }
  ICU4XIsoDateTime local_datetime =
      ICU4XIsoDateTime::create(2022, 8, 25, 0, 0, 0, 0).ok().value();
  ...
  if (metazone_id_return != "amce") {
    std::cout << "Metazone ID not calculated correctly; got "
              << metazone_id_return << std::endl;
    return 1;
  }
  ...
  if (zone_variant_return != "dt") {
    std::cout << "Zone variant not calculated correctly; got "
              << zone_variant_return << std::endl;
    return 1;
  }

  ICU4XGregorianZonedDateTimeFormatter gzdtf =
      ICU4XGregorianZonedDateTimeFormatter::create_with_lengths(
          dp, locale, ICU4XDateLength::Full, ICU4XTimeLength::Full)
          .ok()
          .value();
  ...
  ICU4XZonedDateTimeFormatter zdtf =
      ICU4XZonedDateTimeFormatter::create_with_lengths(
          dp, locale, ICU4XDateLength::Full, ICU4XTimeLength::Full)
          .ok()
          .value();
  out = zdtf.format_datetime_with_custom_time_zone(any_date, time_zone)
            .ok()
            .value();
  std::cout << "Formatted value is " << out << std::endl;
  if (out !=
      "Monday, October 5, 2 Reiwa, 1:33:15\u202fPM Central Daylight Time") {
    std::cout << "Output does not match expected output" << std::endl;
    return 1;
  }
