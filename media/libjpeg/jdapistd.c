  ...
  *width = *width + input_xoffset - *xoffset;
  ...
  for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
    ...
    compptr->downsampled_width = (JDIMENSION)jdiv_round_up(
        (long)(cinfo->output_width * compptr->h_samp_factor),
...
LOCAL(void)
noop_convert(j_decompress_ptr cinfo, JSAMPIMAGE input_buf, JDIMENSION input_row,
...
read_and_discard_scanlines(j_decompress_ptr cinfo, JDIMENSION num_lines)
{
  ...
  for (n = 0; n < num_lines; n++) jpeg_read_scanlines(cinfo, scanlines, 1);

  if (color_convert) cinfo->cconvert->color_convert = color_convert;

  if (color_quantize) cinfo->cquantize->color_quantize = color_quantize;
}
