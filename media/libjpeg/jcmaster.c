...
/* Do computations that are needed before master selection phase */
{
  ...
  for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
    ...
    compptr->width_in_blocks = (JDIMENSION)jdiv_round_up(
        (long)cinfo->_jpeg_width * (long)compptr->h_samp_factor,
        (long)(cinfo->max_h_samp_factor * DCTSIZE));
    compptr->height_in_blocks = (JDIMENSION)jdiv_round_up(
        (long)cinfo->_jpeg_height * (long)compptr->v_samp_factor,
  ...
  cinfo->total_iMCU_rows = (JDIMENSION)jdiv_round_up(
      (long)cinfo->_jpeg_height, (long)(cinfo->max_v_samp_factor * DCTSIZE));
}
...
/* cinfo->comps_in_scan and cinfo->cur_comp_info[] are already set */
{
  ...
  if (cinfo->comps_in_scan == 1) {
    ...
    cinfo->MCUs_per_row = (JDIMENSION)jdiv_round_up(
        (long)cinfo->_jpeg_width, (long)(cinfo->max_h_samp_factor * DCTSIZE));
