...
AsyncHttpsProxySocket::~AsyncHttpsProxySocket() { delete context_; }
...
SocketAddress AsyncHttpsProxySocket::GetRemoteAddress() const { return dest_; }
...
void AsyncHttpsProxySocket::ProcessInput(char* data, size_t* len) {
  ...
  for (size_t pos = start; state_ < PS_TUNNEL && pos < *len;) {
    ...
    if ((length > 0) && (data[start + length - 1] == '\r')) --length;
...
#  error Wrong code for HTTP_STATUS_PROXY_AUTH_REQ
    ...
    switch (HttpAuthenticate(absl::string_view(data + 19, len - 19), proxy_,
      ...
      case HAR_IGNORE:
        ...
        if (!unknown_mechanisms_.empty()) unknown_mechanisms_.append(", ");
